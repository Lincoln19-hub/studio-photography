    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Created {formatDate(invoice.createdAt)} · Due {formatDate(invoice.dueDate)}</p>
        </div>
        <div className="flex gap-2">
          <Link href={`/admin/invoices/${invoice.id}/receipt`} className="btn btn-outline">
            <Printer className="h-4 w-4" /> Print Receipt
          </Link>
          <Link href={`/admin/invoices/${invoice.id}/payment`} className="btn btn-primary">
            + Record Payment
          </Link>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="border-t border-gray-100 bg-gray-50/30 px-6 py-4">
              <div className="space-y-1 text-sm">
                <div className="flex justify-between text-gray-600">
                  <span>Subtotal</span><span>{formatCurrency(invoice.amount)}</span>
                </div>
                <div className="flex justify-between text-gray-600">
                  <span>Tax ({((invoice.taxRate || 0) * 100).toFixed(1)}%)</span><span>{formatCurrency(invoice.taxAmount || 0)}</span>
                </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            {invoice.paidDate && <div className="mt-3 text-xs text-gray-500">Paid on {formatDate(invoice.paidDate)}</div>}
          </div>

          <div className="rounded-xl border border-gray-200 bg-white p-6">
            <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-500">Payment Summary</h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-600">Invoice Total</span><span className="font-medium">{formatCurrency(invoice.total)}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Amount Paid</span><span className="font-medium text-green-600">{formatCurrency(totalPaid)}</span></div>
              ...
              <div className="space-y-3">
                {(invoice.payments || []).map((payment: any) => (
                  <div key={payment.id} className="rounded-lg border border-gray-100 p-3">
                    ...
                    {payment.reference && <div className="mt-1 text-xs text-gray-400">Ref: {payment.reference}</div>}
